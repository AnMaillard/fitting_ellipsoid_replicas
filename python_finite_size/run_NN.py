...
#This file computes the minimal nuclear norm estimator
...
for alpha_ix, alpha in enumerate(alphas):
    ...
    obj = cv.Minimize(cv.norm(X_var, 'nuc'))
    ...
    for ix in range(trials):
        ...
        filename = '../Data/finite_size/evals__min_nuc__d' + str(d) + '__a' + alpha_str + '__t' + str(ix) + '.csv'
        print(filename)
